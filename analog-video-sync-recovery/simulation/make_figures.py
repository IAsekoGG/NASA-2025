...
def note(ax,txt,xy,xytext,color="#C0392B"):
    ax.annotate(txt,xy=xy,xytext=xytext,textcoords="axes fraction",fontsize=9,color=color,
...
note(ax[0,0],"кожні 20 мс (кадр) equalizing-імпульси\nперекидають PC2 → фаза тікає на ±32 мкс\nі «перестрибує» на сусідній рядок",(0.35,0.9),(0.05,0.62))
note(ax[0,1],"зсув 0.6 мкс = затримка LM1881+ФНЧ,\nстала для всіх рядків → невидима",(0.5,0.53),(0.25,0.8),"#2F7A4D")
note(ax[1,0],"картинка «пливе» вбік і рветься —\nхоча період рядка майже правильний",(0.5,0.5),(0.08,0.15))
note(ax[1,1],"рівні краї, колір і горизонт на місці",(0.5,0.6),(0.25,0.15),"#2F7A4D")
...
note(ax[0,0],"сірий = пропадання: шум накачує PC2,\nVCO падає до 10.9 кГц (−30 %)",(0.55,0.15),(0.05,0.75))
note(ax[0,1],"той самий шум, але діапазон VCO\nне пускає далі ніж −2 %",(0.6,0.42),(0.05,0.15),"#2F7A4D")
note(ax[1,0],"32 рядки підряд поза ±5 % →\nдекодер втрачає lock → синій екран",(0.5,0.6),(0.05,0.85))
note(ax[1,1],"макс. 2 % — картинка злегка скошена,\nале декодер тримає",(0.6,0.55),(0.3,0.85),"#2F7A4D")
note(ax[2,0],"синій екран і ще ~50 мс\nхаосу після повернення сигналу",(0.5,0.5),(0.1,0.12))
note(ax[2,1],"сніг замість синього;\n~50 мс «хитання» після повернення, далі картинка на місці",(0.5,0.72),(0.05,0.9),"#2F7A4D")
